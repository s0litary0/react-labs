...
function runParallel() {
    ...
    return getUserP(2)
        .then(user => {
            return Promise.all([getOrdersP(user.id), getRecsP(user.id)]).then(([orders, recommendations]) => ({user, orders, recommendations}));
        })
        .then(({ user, orders, recommendations }) => {
...
function runRaceOptional() {
    ...
    pRace = Promise.race([p1, p2]);
    pRace.then(winner => console.log("race winner: ", winner));
}
